...
public class WaitButton {
  ...
  private int positionX, positionY, width = 32, height = 32;
  private boolean isHovered = false;

  public WaitButton(int screenWidth, int screenHeight) {
    try {
      image = ImageIO.read(getClass().getResourceAsStream("/ui/wait_button/wait_button.png"));
  ...
  public void update(int mouseX, int mouseY) {
    isHovered = (mouseX >= positionX && mouseX <= positionX + width && mouseY >= positionY
        && mouseY <= positionY + height);
  }

  public boolean isClicked(int mouseX, int mouseY) {
    return (mouseX >= positionX && mouseX <= positionX + width && mouseY >= positionY
        && mouseY <= positionY + height);
  }
